...
(function() {
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const funcionVendedorSelectedId = String($('#ComboFuncionVendedor').data('id'));
        const funcionVendedorSelectedText = $('#ComboFuncionVendedor').data('text');

        $('#ComboFuncionVendedor').select2({
            theme: "bootstrap-5",
            language: "es",
            allowClear: true,
            multiple: true,
        ...
        if (funcionVendedorSelectedId && funcionVendedorSelectedText) {
            const selectedIds = funcionVendedorSelectedId.split(','); // Manejo de múltiples valores
            const selectedTexts = funcionVendedorSelectedText.split(',');

            selectedIds.forEach((id, index) => {
                const option = new Option(selectedTexts[index], id, true, true);
                $('#ComboFuncionVendedor').append(option);
            });

            $('#ComboFuncionVendedor').trigger('change');
        }

        const funcionSelectedId = $('#ComboUsuarioSistema').data('id');
        const funcionSelectedText = $('#ComboUsuarioSistema').data('text');

        $('#ComboUsuarioSistema').select2({
            theme: "bootstrap-5",
            language: "es",
            allowClear: true,
            placeholder: 'Selecciona un elemento',
            ajax: {
                url: '/ComboUsuario/ListarUsuario',
                dataType: 'json',
                delay: 250,
                data: function(params) {
                    return {
        ...
        if (funcionSelectedId && funcionSelectedText) {
            ...
            $('#ComboUsuarioSistema').append(preselectedOption).trigger('change');
        }
    });
})();