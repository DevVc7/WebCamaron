...
{
    public class ComboCiudadController(ISeCiudadService seCiudadService) : BaseController
    {
        private readonly ISeCiudadService _seCiudadService = seCiudadService;

        [HttpGet]
        [Authorize]
        public async Task<IActionResult> ListarCiudades(long? idProvincia, string textoContiene, bool esCascada = true)
        {
            try
            {
                if (esCascada && !idProvincia.HasValue)
                {
                    return Ok(Array.Empty<ComboBoxCatalogoModel>());
                }

                var consulta = await _seCiudadService.ConsultarTodos(new()
                {
