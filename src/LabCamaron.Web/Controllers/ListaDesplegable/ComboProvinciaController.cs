...
{
    public class ComboProvinciaController(ISeProvinciaService seProvinciaService) : BaseController
    {
        private readonly ISeProvinciaService _seProvinciaService = seProvinciaService;

        [HttpGet]
        [Authorize]
        public async Task<IActionResult> ListarProvincias(string textoContiene)
        {
            try
            {
                var consulta = await _seProvinciaService.ConsultarTodos(new()
                {
                    SoloActivos = true
                });
